...
# --external-prefix /x/ : references under /x/ are served by something other than the
# export (here: the media library in R2), so they are neither missing nor recoverable.
EXTERNAL = tuple(a.split("=",1)[1] for a in sys.argv[3:] if a.startswith("--external-prefix="))
bad = {}
missing_paths = set()
    ...
    for name in files:
        ...
        for raw in REF.findall(html):
            ...
            if path is None or not path.startswith(LOCAL):
                continue
            if EXTERNAL and path.startswith(EXTERNAL):
                continue
